...
def test_email():
    ...
    with google_api_service(CREDENTIALS, 'gmail', 'v1', SCOPES) as gmail:
        rand_email_id = gmail.users().messages().list(userId='me').execute()['messages'][randint(0, 20)]['id']

        email = gmail.users().messages().get(userId='me', id= rand_email_id, metadataHeaders='full').execute()
    return email
